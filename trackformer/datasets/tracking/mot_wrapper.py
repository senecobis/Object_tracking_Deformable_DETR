...
from .excav_sequence import ExcavSequence
...
class ExcavWrapper(Dataset):
    """A Wrapper for the excav_Sequence class to return multiple sequences."""
    ...
    def __init__(self, split: str, **kwargs) -> None:
        ...
        elif f"Excav-{split}" in train_sequences + test_sequences:
            sequences = [f"Excav-{split}"]
        else:
            raise NotImplementedError("Excav split not available.")

        self._data = []
        for seq in sequences:
            self._data.append(ExcavSequence(seq_name=seq, dets=None, **kwargs))
