...
class ModifiedSequence(Dataset):
    """Multiple Object Tracking (MOT17) Dataset.
    ...
    def __init__(self, root_dir: str = 'data', seq_name: Optional[str] = "EXCAV",
                 dets: str = 'ALL', vis_threshold: float = 0.0, img_transform: Namespace = None) -> None:
        """
        Args:
            seq_name (string): Sequence to take
            vis_threshold (float): Threshold of visibility of persons
        ...
        #self._train_folders = os.listdir(os.path.join(self._data_dir))
        self._test_folders = os.listdir(os.path.join(self._data_dir)+ "/test")
    ...
    def get_seq_path(self) -> str:
        ...
        print("\n full_seq_name", full_seq_name)

        #if self._dets is not None:
        #    full_seq_name = f"{self._seq_name}-{self._dets}"
        return print("\n dir", osp.join(self._data_dir, 'test'))
